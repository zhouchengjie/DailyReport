                    ...
                    switch (column.DataType.ToString())
                    {
                        case "System.String": //字符串类型
                            ...
                            else
                            {
                                if (drValue.Contains("@@@@")) 
                                {
                                    string url = drValue.Substring(drValue.IndexOf("@@@@") + 4);
                                    HSSFHyperlink hssfHyperlink = new HSSFHyperlink(HyperlinkType.Url)
                                    {
                                        Address = url
                                    };
                                    newCell.Hyperlink = hssfHyperlink;

                                    drValue = drValue.Substring(0, drValue.IndexOf("@@@@"));
                                }
            ...
            foreach (DataRow row in dtSource.Rows)
            {
                ...
                foreach (DataColumn column in dtSource.Columns)
                {
                    ...
                    switch (column.DataType.ToString())
                    {
                        case "System.String": //字符串类型
                            double result;
                            if (isNumeric(drValue, out result) && drValue.Length <= 11)
                            {
                            ...
                            else
                            {
                                if (drValue.Contains("@@@@"))
                                {
                                    string text = drValue.Substring(0, drValue.IndexOf("@@@@"));
                                    newCell.SetCellValue(text);

                                    string url = drValue.Substring(drValue.IndexOf("@@@@") + 4);
                                    XSSFHyperlink xssfHyperlink = new XSSFHyperlink(HyperlinkType.Url);
                                    xssfHyperlink.Address = url;
                                    newCell.Hyperlink = xssfHyperlink;

                                    //ICellStyle style = newCell.CellStyle;
                                    //IFont font = style.GetFont(workbook);

                                    //ICellStyle newStyle = workbook.CreateCellStyle();//创建单元格样式
                                    //IFont newfont = workbook.CreateFont();
                                    //newfont.CloneStyleFrom(font);
                                    //newStyle.CloneStyleFrom(style);

                                    //newfont.Underline = FontUnderlineType.SingleAccounting;
                                    ////newfont.Color = HSSFColor.Blue.Index;//设置字体颜色
                                    //newStyle.SetFont(newfont);
                                    //newCell.CellStyle = newStyle;//为单元格设置显示样式

                                    //IFont font = workbook.CreateFont();//创建字体样式
                                    //font.Color =HSSFColor.BlueGrey.Index ;//设置字体颜色
                                    //font.Underline = FontUnderlineType.Single;
                                    //ICellStyle style = workbook.CreateCellStyle();//创建单元格样式
                                    //style.SetFont(font);//设置单元格样式中的字体样式
                                    //newCell.CellStyle = style;//为单元格设置显示样式
                                    break;
                                }
                        ...
                        for (int j = row.FirstCellNum; j <= cellCount; j++)
                        {
                            try
                            {
                                if (row.GetCell(j) != null)
                                {
                                    ...
                                    if (row.GetCell(j).Hyperlink != null)
                                    {
                                        dataRow[j] = row.GetCell(j).Hyperlink.Address;
                                    }
                                }
                            }
